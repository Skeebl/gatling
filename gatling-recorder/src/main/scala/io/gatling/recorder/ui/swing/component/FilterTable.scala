...
class FilterTable extends JPanel with MouseListener {
	...
	def validateCells = {
		...
		for (i <- 0 until model.getRowCount) {
			val modelValue = model.getValueAt(i, 0)
			val emptyMessage = modelValue match {
				//It should not be possible to have a non-empty String there, as typing a value would create a JTextField
				case "" => true
				case tf: JTextField => tf.getText.isEmpty
			}
			if (emptyMessage)
				toRemove = i :: toRemove
			else {
				val textField = modelValue.asInstanceOf[JTextField]
				if (!isValidRegex(i)) {
